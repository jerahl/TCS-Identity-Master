...
/**
 * Fire the Adaxes AD reconciler on demand from the web, without blocking the
 * request. The sync does a live AD lookup per person, so it can run for minutes —
 * far too long to run synchronously in an HTTP request like the bounded
 ...
 * unless ADAXES_RUN_ENABLED=true, and the host must grant the web user (www-data)
 * a NOPASSWD sudoers rule for exactly that command (deploy/idm-adaxes-run.sudoers).
 * Never throws — start() returns a result envelope. The runner is injectable so
 * the logic is unit-testable without root.
 */
final class AdaxesRunService
{
    ...
    public function __construct(
        ...
        $this->enabled = $enabled ?? Config::bool('ADAXES_RUN_ENABLED', false);
        $this->unit = trim($unit ?? (string) Config::get('ADAXES_SERVICE_UNIT', 'idm-adaxes-sync.service'));
        $this->sudo = (string) Config::get('ADAXES_SUDO_BIN', 'sudo');
        $this->systemctl = (string) Config::get('ADAXES_SYSTEMCTL_BIN', 'systemctl');
        $this->timeout = $timeout ?? max(5, Config::int('ADAXES_RUN_TIMEOUT', 15));
    ...
    /**
     * Start the AD sync in the background. Never throws — always a result envelope.
     *
    ...
    {
        if (!$this->enabled) {
            return ['ok' => false, 'error' => 'On-demand AD sync is disabled — set ADAXES_RUN_ENABLED=true.', 'output' => ''];
        }
        ...
        try {
            ...
            error_log('[idm] adaxes run: ' . $e->getMessage());
        ...
        if ($code !== 0) {
            $hint = $out !== '' ? $out : 'systemctl exited with code ' . $code
                . ' — check the sudoers rule for the web user (deploy/idm-adaxes-run.sudoers).';
