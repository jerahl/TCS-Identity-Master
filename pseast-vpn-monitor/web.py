...
class SnapshotCache:
    ...
    each browser tab; this coalesces concurrent requests behind one refresh.
    """

    def __init__(self, cfg, mock, fixtures_dir):
        self._cfg = cfg
        self._mock = mock
...
def serve(cfg: dict, mock: bool = False, fixtures_dir: str = monitor.DEFAULT_FIXTURES) -> int:
    ...
    cache = SnapshotCache(cfg, mock, fixtures_dir)

    httpd = ThreadingHTTPServer((addr, port), make_handler(cache))
    where = f"http://{addr}:{port}/"
    print(f"pseast-vpn-monitor serving {where}  (mock={mock}, poll={cache._ttl}s)")
