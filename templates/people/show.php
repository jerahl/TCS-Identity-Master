...
<div class="detail">
  ...
  <!--
    ...
    public/assets/js/person-live-panels.js, which swaps the result into #adaxes-live.
  ...
  <div class="panel" style="margin-top:18px;">
    ...
    <?php if (!empty($adaxesConfigured)): ?>
      <div id="adaxes-live" class="adaxes-live" data-live-url="<?= e($adaxesUrl) ?>"
           data-live-label="Checking Active Directory…"
           data-live-error="Could not load the Active Directory check.">
        <div class="live-panel-loading identity-note" style="margin-bottom:0;">
  ...
  <!--
    Live Google Workspace correlation + direct provisioning (bypasses OneSync).
    The correlation is a live remote lookup (GAM / the Directory API) that can be
    slow, so — like the AD panel — we render the panel immediately with a loading
    indicator and fetch the comparison over AJAX (GET /people/{id}/google), which
    person-live-panels.js swaps into #google-live. When direct provisioning isn't
    configured there's nothing to look up, so that (static) state renders inline.
  -->
  <div class="panel" style="margin-top:18px;" id="google">
    <div class="panel__head">
      <h2 class="panel__title">Google Workspace <span class="muted" style="font-weight:500;">(live · direct)</span></h2>
      <span class="panel__note">— provisioned directly from the golden record, bypassing OneSync</span>
    </div>

    <?php if (!empty($googleConfigured)): ?>
      <div id="google-live" data-live-url="<?= e($googleUrl) ?>"
           data-live-label="Waiting for Google (GAM)…"
           data-live-error="Could not load the Google Workspace check.">
        <div class="live-panel-loading identity-note" style="margin-bottom:0;">
          <span class="spinner" aria-hidden="true"></span>
          <span>Waiting for Google (GAM)…</span>
        </div>
      </div>
    <?php else: ?>
      <?= \App\View\View::partial('people/_google', ['google' => $google, 'verdict' => $verdict, 'p' => $p, 'canEdit' => $canEdit ?? false, 'csrf' => $csrf ?? '']) ?>
    <?php endif; ?>
  </div>
